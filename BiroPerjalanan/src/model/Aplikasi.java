...
public class Aplikasi {
    ...
    public ArrayList<TempatWisata> getTempatWisataFromFile() {
        File f = new File("tempat.txt");
        ArrayList<TempatWisata> tm = new ArrayList<>();
        if (f.exists()) {
            try {
                FileInputStream fis = new FileInputStream(f);
                ObjectInputStream ois = new ObjectInputStream(fis);
                tm = (ArrayList<TempatWisata>) ois.readObject();
            } catch (Exception e) {
                System.out.println(e.getMessage());
            }
        }
    ...
    public ArrayList<PaketWisata> getPaketWisataFromFile() {
        File f = new File("paket.txt");
        ArrayList<PaketWisata> pk = new ArrayList<>();
        if (f.exists()) {
            try {
                FileInputStream fis = new FileInputStream(f);
                ObjectInputStream ois = new ObjectInputStream(fis);
                pk = (ArrayList<PaketWisata>) ois.readObject();
            } catch (Exception e) {
                System.out.println(e.getMessage());
            }
        }
    ...
    public String tampilTempat() {
        ArrayList<TempatWisata> tw = new ArrayList<>();
        for (TempatWisata tm : daftarTempatWisata) {
            tw.add(tm);
        }
        return tw.toString();
    }

    public String tampilPaket() {
        ArrayList<PaketWisata> pk = new ArrayList<>();
    ...
    public Petugas cekLogin(String user, String pass) {
        Petugas c = null;
        for (Petugas p : daftarPetugas) {
            if ((p.getUserName().equals(user)) && (p.getPass().equals(pass))) {
                c = p;
            }
        }
        return c;
